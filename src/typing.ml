...
let new_tyvar mark =
  let n = !var_ref in
  incr var_ref;
  T.Var ("tyvar" ^ string_of_int n ^ "_" ^ mark)

let ty_eq t1 t2 = C.Conj [C.Subtype (t1, t2); C.Subtype (t2, t1)]
let pattern_vars = function
  | P.Var (var, _) -> [var]
  | P.Struct (vars, _) -> vars

exception Duplicate_key

let extend_env env pairs =
  let extra =
    match BatMap.of_alist pairs with
    | `Duplicate_key key ->
...
let rec type_judge env = function
  ...
  | E.Struct exprs ->
     ...
     (T.Struct typs, C.Conj cons)
  | E.App (expr, exprs) ->
     let (typ, con) = type_judge env expr in
     let (typs, cons) = collect_type_judgement env exprs in
     let alpha = new_tyvar "app_alpha" in
     let beta = new_tyvar "app_beta" in
     let gen_alpha i = new_tyvar ("app_alpha" ^ string_of_int i) in
     let alphas = List.map (List.range 0 (List.length exprs)) ~f:gen_alpha in
     let eq = ty_eq typ (T.Fun (alphas, alpha)) in
     let subset = C.Subtype (beta, alpha) in
     let subsets = List.map2_exn typs alphas ~f:(fun t a -> C.Subtype (t, a)) in
     (beta, C.Conj ((eq::subset::subsets) @ (con::cons)))
  | E.Abs (args, body) ->
     ...
     (typ, ty_eq typ (T.Constraint (T.Fun (arg_types, typb), conb)))
  | E.Let (var, e1, e2) ->
     let (typ1, con1) = type_judge env e1 in
     let (typ2, con2) = type_judge (extend_env env [(var, typ1)]) e2 in
     (typ2, C.Conj [con1; con2])
  | E.Letrec (binds, expr) ->
     let (vars, exprs) = List.unzip binds in
  ...
  | E.Case (expr, branches) ->
     ...
     let pattern_judge env =
       let go pat_expr guard =
         ...
         let typg_true = ty_eq typg (T.Val (V.Atom "true")) in
         (typp, C.Conj [conp; cong; typg_true])
       in
       function
       | P.Var (var, guard) ->
          go (E.Var var) guard
       | P.Struct (vars, guard) ->
          go (E.Struct (List.map vars ~f:(fun var -> E.Var var))) guard
     in
     let branch_constraint i (pat, body) =
       ...
       C.Conj [ty_eq beta betab; ty_eq typ alphap; conp; conb]
     in
