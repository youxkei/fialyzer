...
type t =
  | Bottom
  | Mapping of type_expr BatMap.Make(String).t
[@@deriving compare]

let equal sol1 sol2 = match sol1, sol2 with
  | Bottom, Bottom -> true
  | Mapping map1, Mapping map2 -> BatMap.equal Type.equal map1 map2
  | _, _ -> false
...
let rec init = function
  | C.Empty -> empty
  | C.Conj cs -> List.map cs ~f:init |> merge_all
  | C.Disj cs -> List.map cs ~f:init |> merge_all
  | C.Subtype (e1, e2) ->
     let rec ee = function
       | T.Var var -> Map.singleton (module String) var (T.Any)
       | T.Struct es -> List.map es ~f:ee |> merge_all
       | T.Fun (es, e) -> List.map (e::es) ~f:ee |> merge_all
       | T.Union (e1, e2) -> merge_all [ee e1; ee e2]
       | T.Constraint (e, c) -> merge_all [ee e; init c]
       | _ -> empty
     in
